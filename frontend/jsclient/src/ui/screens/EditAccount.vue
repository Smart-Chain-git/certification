      ...
      <v-flex lg6 md7 sm8 xs11>
        <Card>
          ...
          <v-card-text class="pa-0" lg3>
            <EditFormRow v-if="access === 'creating' || access === 'adminEditing'"
                         :title="$t('account.edit.login')+' *'" :editable="true">
              <EditFormTitleEdit :placeholder="$t('account.edit.login')" color="var(--var-color-blue-sword)"
                                 v-model.trim="draft.login"/>
            </EditFormRow>
            <EditFormRow v-else :title="$t('account.edit.login')" :editable="false" :value="draft.login"/>

            <EditFormRow v-if="access === 'creating' || access === 'adminEditing'"
                         :title="$t('account.edit.email')+' *'" :editable="true">
              <EditFormTitleEdit :placeholder="$t('account.edit.email')" color="var(--var-color-blue-sword)"
                                 v-model.trim="draft.email"/>
            </EditFormRow>
            <EditFormRow v-else :title="$t('account.edit.email')" :editable="false" :value="draft.email"/>

            <EditFormRow v-if="access === 'creating' || access === 'adminEditing'" :title="$t('account.edit.company')"
                         :editable="true">
              <EditFormTitleEdit :placeholder="$t('account.edit.company')" color="var(--var-color-blue-sword)"
                                 v-model.trim="draft.company"/>
            </EditFormRow>
            <EditFormRow v-else :title="$t('account.edit.company')" :editable="false" :value="draft.company"/>

            <EditFormRow v-if="access === 'creating' || access === 'adminEditing'" :title="$t('account.edit.country')"
                         :editable="true">
              <EditFormTitleEdit :placeholder="$t('account.edit.country')" color="var(--var-color-blue-sword)"
                                 v-model.trim="draft.country"/>
            </EditFormRow>
            <EditFormRow v-else :title="$t('account.edit.country')" :editable="false" :value="draft.country"/>

            <EditFormRow v-if="access === 'creating' || access === 'adminEditing'" :title="$t('account.edit.admin')"
                         :editable="true">
              <v-checkbox v-model="draft.isAdmin"/>
            </EditFormRow>
            <EditFormRow v-else :title="$t('account.edit.profile')" :editable="false"
                         :value="draft.isAdmin ? $t('account.edit.admin') : $t('account.edit.noAdmin')"/>

            <EditFormRow v-if="access === 'creating' || access === 'adminEditing'"
                         :title="$t('account.edit.TEZOSPubKey')" :editable="true">
              <EditFormTitleEdit :placeholder="$t('account.edit.TEZOSPubKey')" color="var(--var-color-blue-sword)"
                                 v-model.trim="draft.publicKey"/>
            </EditFormRow>
            <EditFormRow v-else :title="$t('account.edit.TEZOSPubKey')" :editable="false" :value="draft.publicKey"/>


            <EditFormRow v-if="access === 'creating' || access === 'adminEditing'"
                         :title="$t('account.edit.TEZOSAccount')" :editable="true">
              <EditFormTitleEdit :placeholder="$t('account.edit.TEZOSAccount')" color="var(--var-color-blue-sword)"
                                 v-model.trim="draft.hash"/>
            </EditFormRow>
            ...
            <EditFormRow :title="$t('account.edit.fullName') +' *'" :editable="true">
              <EditFormTitleEdit :placeholder="$t('account.edit.fullName')" v-model.trim="draft.fullName"
            ...
            <v-card-actions class="navigation pt-8 pb-4">
              ...
              <v-flex class="align-right">
                ...
                <IconButton v-if="access === 'creating'" color="var(--var-color-blue-sword)" @click="create"
                            :disabled="!canCreate" leftIcon="add_circle_outline">
                  {{ $t("account.list.add") }}
                </IconButton>
                <IconButton v-else color="var(--var-color-blue-sword)" @click="save" :disabled="!canSave"
                            leftIcon="save">
                  {{ $t("account.edit.save") }}
                </IconButton>
              </v-flex>
    ...
    this.$modules.accounts.createAccount(create).then(() => {
      if (this.$modules.accounts.getHttpStatus() !== 200) {
        if (this.$modules.accounts.getHttpStatus() === 409) {
          if (this.$modules.accounts.getHttpErrorMessage.includes("email dup key")) {
            this.fail("errors.account.duplicate.email")
          } else {
            this.fail("errors.account.duplicate.login")
          }
        } else {
          this.fail("errors.back.generic")
        }
